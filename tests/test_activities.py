...
async def test_upload_pdf(env, s3, settings, pdf_bytes, tmp_path):
    ...
    result = await env.run(upload_pdf, UploadPdfInput(local_path=str(source), key="report.pdf"))
...
async def test_parse_pdf(env, pdf_bytes, tmp_path):
    ...
    result = await env.run(parse_pdf, ParsePdfInput(local_path=str(source)))

    assert "Quarterly Report" in result.markdown


async def test_upload_md(env, s3, settings):
    result = await env.run(upload_md, UploadMdInput(markdown="# heading", key="report.md"))
...
async def test_activities_round_trip_through_both_buckets(env, s3, settings, pdf_bytes, tmp_path):
    ...
    await env.run(upload_pdf, UploadPdfInput(local_path=str(source), key="report.pdf"))
    fetched = await env.run(download_pdf, DownloadPdfInput(key="report.pdf"))
    parsed = await env.run(parse_pdf, ParsePdfInput(local_path=fetched.local_path))
    await env.run(upload_md, UploadMdInput(markdown=parsed.markdown, key="report.md"))
    final = await env.run(download_md, DownloadMdInput(key="report.md"))
